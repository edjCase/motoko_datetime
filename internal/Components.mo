...
module Module {
  ...
  public func toTextFormatted(
    ...
    var text = "";
    for (token in List.values(tokens)) {
      text #= switch (token) {
        case (#text(t)) t;
  ...
  public func fromText(text : Text, format : Text, locale : ?Locale) : ?FromTextResult {
    ...
    let tokenOrTextBuffer : List.List<TokenOrText> = parseFormatText(format);
    let extractedValueHelper = ExtractedValueHelper();
    for (tokenOrText in List.values(tokenOrTextBuffer)) {
      switch (tokenOrText) {
        case (#text(t)) {
          // skip text
  ...
  private class ExtractedValueHelper() {
    let extractedValues = List.empty<ExtractedValue>();
    ...
    public func add(value : ExtractedValue) : () {
      List.add(extractedValues, value);
    };
    ...
    public func resolve() : ?(Components, TimeZoneDescriptor) {
      ...
      for (value in List.values(extractedValues)) {
        switch (value) {
      ...
      label l loop {
        let ?lastValue = List.removeLast(extractedValues) else break l;
    ...
    func updateState(state : State, value : ExtractedValue) : State {
      ...
      let newState = switch (value) {
        ...
        case (#hour12(h)) {
          switch (state.isPM) {
            case (null) Runtime.trap("Meridiem is required when using 12 hour clock"); // isPM is prefetched, TODO make better
            case (?true) {
        ...
        case (#dayOfYear(d)) {
          let ?year = state.year else Runtime.trap("Year is required when using day of year"); // Year is prefetched, TODO make better
          var month = 1;
          var remainingDays = d;
          label l loop {
  ...
  public func addTime(components : Components, nanoseconds : Time.Time) : Components {
    if (not isValid(components)) {
      return Runtime.trap("Invalid components: " # debug_show (components));
    };
    if (nanoseconds < 0) {
  ...
  private func parseFormatText(format : Text) : List.List<TokenOrText> {
    var startIndex = 0;
    var length = 1;
    let tokenBuffer = List.empty<TokenOrText>();
    var bracketDepth = 0;
    var tokenMatches = tokens;
    let formatBuffer = List.fromIter<Char>(Text.toIter(format));

    let appendTextAndReset = func(buffer : List.List<Char>, skipLastChar : Bool) {
      if (skipLastChar) {
        if (List.size(buffer) > 1) {
          // If left over text, append it
          let size : Nat = List.size(buffer) - 1;
          let text = Text.fromIter(Iter.take(List.values(buffer), size));
          List.add(tokenBuffer, #text(text));
        };
      } else {
        let text = Text.fromIter(List.values(buffer));
        List.add(tokenBuffer, #text(text));
      };
      startIndex += length;
      length := 1;
    ...
    let matchAppendAndReset = func(exact : Bool) {
      ...
      let buffer = List.values(formatBuffer)
      |> Iter.drop(_, startIndex)
      |> Iter.take(_, length)
      |> List.fromIter<Char>(_);
      let value = Text.fromIter(List.values(buffer));
      ...
      switch (fitleredTokens.size()) {
        case (0) {
          ...
          List.add(tokenBuffer, #text(value));
          startIndex += length;
          length := 1;
          tokenMatches := tokens;
        };
        case (1) {
          ...
          List.add(tokenBuffer, #token(token));
          startIndex += length;
          length := 1;
          tokenMatches := tokens;
        };
    ...
    label l loop {
      if (length + startIndex > List.size(formatBuffer)) {
      ...
      let subBuffer : List.List<Char> = formatBuffer
      |> List.values(_)
      |> Iter.drop(_, startIndex)
      |> Iter.take(_, length)
      |> List.fromIter(_);
      let nextChar = List.last(subBuffer);
  ...
  public func throwIfNotValid(components : Components) : () {
    if (not isValid(components)) {
      Runtime.trap("Invalid datetime components: " # debug_show (components));
    };
  };
  ...
  public func daysInMonth(month : Nat, isLeapYear : Bool) : Nat {
    switch (month) {
      ...
      case _ Runtime.trap("Invalid month: " # Nat.toText(month));
    };
  };
  ...
  private func requireLocale(locale : ?Locale) : Locale {
    switch (locale) {
      case (?l) l;
      case (null) Runtime.trap("Locale is required to output the specified date format");
    };
  };
  ...
  private func getEra(time : Time.Time, locale : Locale) : Era {
    ...
    switch (Array.find<Era>(locale.eras, eraFilter)) {
      case (?era) era;
      case (null) Runtime.trap("No era found for time and locale. Time: " # debug_show (time) # ", Locale: " # locale.id);
    };
  };
  ...
  private func take<T>(iter : Iter.Iter<T>, count : Nat) : (List.List<T>, Iter.Iter<T>) {
    let takenBuffer = List.empty<T>();
    var i = 0;
    label l loop {
      if (i >= count) {
        break l;
  ...
  private func extractNat(text : Text, maxDigits : Nat, strict : Bool) : ?(Nat, Text) {
    ...
    label l for (c in List.values(taken)) {
      let digit = switch (c) {
        case ('0') 0;
        case ('1') 1;
        case ('2') 2;
      ...
      List.add(values, digit);
      digitCount += 1;
    };
    type Acc = { value : Nat; i : Nat };
    let natValue = List.foldRight(
      values,
      { value = 0; i = 0 },
    ...
    let r = if (digitCount >= List.size(taken)) {
      Text.fromIter(remainingText);
    } else {
      let unusedTakenLength : Nat = List.size(taken) - digitCount;
      let unusedTakenText = List.values(taken)
      |> Iter.drop(_, digitCount)
      |> Iter.take(_, unusedTakenLength)
      |> Text.fromIter(_);
